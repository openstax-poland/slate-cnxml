...
var Slate__namespace = /*#__PURE__*/_interopNamespace(Slate);
...
function normalizeTextBoundaries(editor, options) {
    ...
    if (end != null && !Slate.Editor.isEnd(editor, end, nodePath.current)) {
        if (ends === 'unwrap') {
            ...
            Slate.Transforms.liftNodes(editor, {
                at: Slate.Editor.range(editor, Slate.Path.next(end.path), nodePath.current),
                match: Slate.Text.isText,
            });
        }
        else {
            Slate.Transforms.delete(editor, { at: Slate.Editor.range(editor, end, at) });
        }
    }
    ...
    if (start != null && !Slate.Editor.isStart(editor, start, nodePath.current)) {
        if (ends === 'unwrap') {
            ...
            Slate.Transforms.delete(editor, { at: Slate.Editor.range(editor, nodePath.current, start) });
        }
    }
    ...
    const node = Slate.Node.get(editor, nodePath.current);
    ...
    for (const [index, child] of enumerate(node.children, affinity === 'end')) {
        if (!Slate.Text.isText(child)) {
            return affinity === 'start'
                ? Slate.Editor.start(editor, [...at, index])
                : Slate.Editor.end(editor, [...at, index]);
        }
...
function normalizeLine(editor, at) {
    const unwrap = [];
    for (const [child, path] of Slate__namespace.Node.children(editor, at)) {
        if (Slate__namespace.Element.isElement(child) && !editor.isInline(child)) {
            unwrap.push(Slate.Editor.pathRef(editor, path));
        }
    }
    normalizeWhiteSpace(editor, at);
    for (const path of unwrap) {
        Slate.Transforms.liftNodes(editor, { at: path.unref() });
    }
}
