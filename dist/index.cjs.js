...
function tgroup$1(editor, el, at) {
    ...
    // Process content
    const path = Slate.Editor.pathRef(editor, [...at, 0]);
    if (header != null) {
...
function tableBody(editor, el, at, start = [...at, 0]) {
    ...
    const columns = cnxDesigner.Table.columns(editor, at);
    const table = Array(rows)
        .fill(0)
        .map(() => Array(columns.columns.length).fill(false));
    let row = 0;
    const path = Slate.Editor.pathRef(editor, start);
    for (const child of el.children) {
        if (child.namespaceURI !== CNXML_NAMESPACE) {
            editor.reportError('unknown-element', {
                namespace: child.namespaceURI,
...
function tableRow(editor, el, at, table, row) {
    ...
    let column = 1;
    const entry = Slate.Editor.pathRef(editor, [...at, 0]);
    for (const child of el.children) {
        if (child.namespaceURI !== CNXML_NAMESPACE) {
            editor.reportError('unknown-element', {
                namespace: child.namespaceURI,
...
const SERIALIZERS = [
    ...
    [cnxDesigner.Table.isCell, entry],
    [cnxDesigner.Table.isFooter, tfoot],
    [cnxDesigner.Table.isGroup, tgroup],
    [cnxDesigner.Table.isHeader, thead],
    [cnxDesigner.Table.isRow, row],
    [cnxDesigner.Table.isSummary, tablesummary],
    [cnxDesigner.Table.isTable, table],
...
function table(node, attrs, children) {
    let summary;
    const summaryNode = node.children.find(cnxDesigner.Table.isSummary);
    if (summaryNode != null && isPlainText(summaryNode)) {
        summary = Slate.Node.string(summaryNode);
    }
