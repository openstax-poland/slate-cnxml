...
import { WithClasses, List, MediaUse, Table, Admonition, AltText, Audio, Caption, Code, Commentary, CrossReference, Definition, DefinitionExample, DefinitionTerm, DocumentReference, Exercise, Figure, Footnote, Foreign, Glossary, Image, Link, ListItem, Meaning, Media, Paragraph, Preformat, ProcessingInstruction, Problem, Proof, Quotation, Rule, RuleExample, Section, SeeAlso, Solution, Statement, Term, Title, Video } from 'cnx-designer';
...
function normalizeTextBoundaries(editor, options) {
    ...
    let node = Node$2.ancestor(editor, nodePath.current);
    ...
    // elements
    node = Node$2.ancestor(editor, nodePath.current);
...
function collapseAdjacentText(editor, at) {
    const node = Node$2.ancestor(editor, at);
 ...
 */
function buildElement(editor, el, at, template, context, withChildren = true) {
...
const TABLE = { title, tgroup: tgroup$1, caption };
const TGROUP = {
    thead: theadfoot('table_header'),
    tfoot: theadfoot('table_footer'),
};
/** Deserialize a <table> */
function table$1(editor, el, at) {
    var _a;
    buildElement(editor, el, at, {
        type: 'table',
    }, TABLE);
    const node = Slate.Node.get(editor, at);
    const summary = (_a = requireAttribute(editor, el, 'summary')) !== null && _a !== void 0 ? _a : '';
...
function tgroup$1(editor, el, at) {
    var _a, _b;
    const cols = (_a = numericAttribute(editor, el, 'cols')) !== null && _a !== void 0 ? _a : 1;
    const columns = [];
    ...
    for (const child of el.children) {
        ...
        switch (child.localName) {
            case 'colspec': {
                const colname = (_b = child.getAttribute('colname')) !== null && _b !== void 0 ? _b : null;
...
function theadfoot(type) {
    return function deserializer(editor, el, at) {
        ...
        for (const child of el.children) {
            ...
            switch (child.localName) {
                case 'colspec': {
                    const colname = (_a = child.getAttribute('colname')) !== null && _a !== void 0 ? _a : null;
                    const colnum = (_b = numericAttribute(editor, child, 'colnum', false)) !== null && _b !== void 0 ? _b : columns.length + 1;
 ...
 */
function tableBody(editor, el, at, start = [...at, 0]) {
    let rows = 0;
    for (const child of el.children) {
        if (child.namespaceURI === CNXML_NAMESPACE && child.localName === 'row')
            rows += 1;
    }
    const columns = Table.columns(editor, at);
    const table = Array(rows)
        .fill(0)
        .map(() => Array(columns.columns.length).fill(false));
    let row = 0;
...
function tableRow(editor, el, at, table, row) {
    var _a;
    buildElement(editor, el, at, {
        type: 'table_row',
    }, {}, false);
    ...
    for (const child of el.children) {
        ...
        const colnameFinal = namest !== null && namest !== void 0 ? namest : colname;
        const morerows = (_a = numericAttribute(editor, child, 'morerows', false)) !== null && _a !== void 0 ? _a : 0;
...
function requireAttribute(editor, el, name) {
 ...
 * a number
 */
function numericAttribute(editor, el, name, required = true) {
    const repr = required
 ...
 */
function setAttribute(editor, el, name, values, required = true) {
...
function table(node, attrs, children) {
    let summary;
    const summaryNode = node.children.find(Table.isSummary);
    if (summaryNode != null && isPlainText(summaryNode)) {
        summary = Node$2.string(summaryNode);
    }
    else if (summaryNode == null) {
        summary = "";
    }
    return jsx("table", { xmlns: CNXML_NAMESPACE, summary: summary, ...attrs, children: children });
}
function tablesummary(node, attrs, children) {
    // If summary contains only plain text it will be instead emitted in
    // a summary attribute on a <table> element.
    if (isPlainText(node)) {
        return null;
    }
    return jsx("summary", { xmlns: EDITING_NAMESPACE, ...attrs, children: children });
}
function tgroup(node, attrs, children) {
    if (!Array.isArray(children))
        throw new Error("invalid children of table_group");
    let thead = null;
    let tfoot = null;
    const tbody = [];
    for (const child of children) {
        if (!Node$1.isElement(child))
            throw new Error("invalid child of table_group");
    ...
    const spans = node.spans.map(span => jsx("spanspec", { xmlns: CNXML_NAMESPACE, spanname: span.name, namest: span.start, nameend: span.end }));
    return jsxs("tgroup", { xmlns: CNXML_NAMESPACE, cols: node.columns.length, ...attrs, children: [node.columns.map(colspec), spans, thead, tfoot, jsx("tbody", { children: tbody })] });
}
function colspec(column, index) {
    // Skip columns with default values
    if (column.name == null)
        return null;
    return jsx("colspec", { xmlns: CNXML_NAMESPACE, colnum: index + 1, colname: column.name });
}
function thead(node, attrs, children) {
    var _a;
    return jsxs("thead", { xmlns: CNXML_NAMESPACE, ...attrs, children: [(_a = node.columns) === null || _a === void 0 ? void 0 : _a.map(colspec), children] });
}
function tfoot(node, attrs, children) {
    var _a;
    return jsxs("tfoot", { xmlns: CNXML_NAMESPACE, ...attrs, children: [(_a = node.columns) === null || _a === void 0 ? void 0 : _a.map(colspec), children] });
}
function row(node, attrs, children) {
    return jsx("row", { xmlns: CNXML_NAMESPACE, ...attrs, children: children });
}
function entry(node, attrs, children) {
    var _a;
    const rows = (_a = node.rows) !== null && _a !== void 0 ? _a : 1;
    return jsx("entry", { xmlns: CNXML_NAMESPACE, morerows: rows > 1 ? rows - 1 : undefined, ...cellPosition(node), ...attrs, children: children });
}
...
export { BLOCK, CMLNLE_NAMESPACE, CNXML_NAMESPACE, CXLXT_NAMESPACE, EDITING_NAMESPACE, INLINE, jsx$1 as JSX, LINE, MIXED, Node$1 as Node, XMLNS_NAMESPACE, XML_NAMESPACE, buildElement, children, createElement, deserialize, jsx, jsxs, normalizeBlock, normalizeLine, normalizeMixed, normalizeVoid, render, serialize };
