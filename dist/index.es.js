...
import { Element, Node as Node$2, Editor, Transforms, Text, Path } from 'slate';
import { WithClasses, List, MediaUse, Admonition, AltText, Audio, Caption, Code, Commentary, CrossReference, Definition, DefinitionExample, DefinitionTerm, DocumentReference, Exercise, Figure, Footnote, Foreign, Glossary, Image, Link, ListItem, Meaning, Media, Paragraph, Preformat, ProcessingInstruction, Problem, Proof, Quotation, Rule, RuleExample, Section, SeeAlso, Solution, Statement, Term, Title, Video } from 'cnx-designer';
...
function normalizeWhiteSpace(editor, at) {
    const node = Node$2.get(editor, at);
...
function normalizeTextBoundaries(editor, options) {
    ...
    for (const [child, path] of Node$2.children(editor, nodePath.current, { reverse: true })) {
    ...
    const node = Node$2.get(editor, nodePath.current);
...
function regexReplace(editor, path, re, replacer) {
    Editor.withoutNormalizing(editor, () => {
        const node = Node$2.get(editor, path);
        if (!Text.isText(node)) {
...
/** Media items */
const MEDIA = { audio: mediaItem$1, image: mediaItem$1, video: mediaItem$1 };
/** Contents of a rule */
const RULE = {
...
function serialize(editor, doc, options) {
    ...
    const glossary = Glossary.isGlossary(doc.content[doc.content.length - 1])
        ? content.pop()
        : null;
    const document = render(jsxs("document", Object.assign({ xmlns: CNXML_NAMESPACE, "cnxml-version": doc.version, id: doc.moduleId, "module-id": doc.moduleId, xmlLang: doc.language, class: (_a = doc.classes) === null || _a === void 0 ? void 0 : _a.join(' ') }, { children: [jsx("title", { children: doc.title }, void 0), jsx("content", { children: content }, void 0), glossary] })));
    if (format === 'dom') {
        return document;
    }
...
function applyTextStyle(style, value, node) {
    switch (style) {
        case 'emphasis':
            return value
                ? jsx("emphasis", Object.assign({ xmlns: CNXML_NAMESPACE, effect: "italics" }, { children: node }))
                : node;
        case 'strong':
            return value
                ? jsx("emphasis", Object.assign({ xmlns: CNXML_NAMESPACE, effect: "bold" }, { children: node }))
                : node;
        case 'underline':
            return value
                ? jsx("emphasis", Object.assign({ xmlns: CNXML_NAMESPACE, effect: "underline" }, { children: node }))
                : node;
        case 'position':
            return value === 'superscript'
                ? jsx("sup", Object.assign({ xmlns: CNXML_NAMESPACE }, { children: node }))
                : value === 'subscript'
                    ? jsx("sub", Object.assign({ xmlns: CNXML_NAMESPACE }, { children: node }))
                    : node;
    }
}
...
function makeSerializer(Tag, namespace = CNXML_NAMESPACE) {
    return function serializer(node, attrs, children) {
        return jsx(Tag, Object.assign({ xmlns: namespace }, attrs, { children: children }));
    };
}
function admonition(node, attrs, children) {
    return jsx("note", Object.assign({ xmlns: CNXML_NAMESPACE, type: node.kind }, attrs, { children: children }));
}
function altText(node, attrs, children) {
    ...
    return jsx("alt-text", Object.assign({ xmlns: EDITING_NAMESPACE }, attrs, { children: children }));
}
function code(node, attrs, children) {
    return jsx("code", Object.assign({ xmlns: CNXML_NAMESPACE, display: node.placement === 'block' ? 'block' : undefined, lang: node.language }, attrs, { children: children }));
}
function docref(node, attrs, children) {
    return jsx("link", Object.assign({ xmlns: CNXML_NAMESPACE, document: node.document }, attrs, { children: children }));
}
function figure(node, attrs, children) {
    ...
    return jsx("figure", Object.assign({ xmlns: CNXML_NAMESPACE }, attrs, { children: mapChild(children) }));
}
function foreign(node, attrs, children) {
    return jsx("foreign", Object.assign({ xmlns: CNXML_NAMESPACE, xmlLang: node.language }, attrs, { children: children }));
}
function link(node, attrs, children) {
    return jsx("link", Object.assign({ xmlns: CNXML_NAMESPACE, url: node.url }, attrs, { children: children }));
}
function list(node, attrs, children) {
    function mapChild(child) {
        ...
        if ((child.name.namespace == null || child.name.namespace === CNXML_NAMESPACE)
            ...
            return jsx("item", Object.assign({ xmlns: CNXML_NAMESPACE }, { children: child }));
        }
        return child;
    }
    return jsx("list", Object.assign({ xmlns: CNXML_NAMESPACE, "list-type": node.style === 'enumerated' ? node.style : undefined, "bullet-style": (node.bullet !== 'bullet' ? node.bullet : undefined), "number-style": (node.numberStyle !== 'arabic' ? node.numberStyle : undefined), "start-value": (node.start !== 1 ? node.start : undefined) }, attrs, { children: mapChild(children) }));
}
function media(node, attrs, children) {
    ...
    if (altNode != null && isPlainText(altNode)) {
        alt = Node$2.string(altNode);
    }
    return jsx("media", Object.assign({ xmlns: CNXML_NAMESPACE, alt: alt }, attrs, { children: children }));
}
function mediaItem(node, attrs, children, ctx) {
    const Tag = node.type.slice(6);
    return jsx(Tag, Object.assign({ xmlns: CNXML_NAMESPACE, src: node.src, "mime-type": ctx.mediaMime(node), for: node.intendedUse === 'all' ? undefined : node.intendedUse }, attrs, { children: children }));
}
function processingInstruction(node) {
    return { target: node.target, value: node.value };
}
function rule(node, attrs, children) {
    return jsx("rule", Object.assign({ xmlns: CNXML_NAMESPACE, type: node.kind === 'rule' ? undefined : node.kind }, attrs, { children: children }));
}
function term(node, attrs, children) {
    ...
    return jsx("term", Object.assign({ xmlns: CNXML_NAMESPACE, cxlxtIndex: node.index, cmlnleReference: node.reference }, nameIndexAttributes, attrs, { children: children }));
}
function xref(node, attrs, children) {
    var _a;
    return jsx("link", Object.assign({ xmlns: CNXML_NAMESPACE, "target-id": node.target, document: (_a = node.document) !== null && _a !== void 0 ? _a : undefined, cmlnleCase: node.case }, attrs, { children: children }));
}

export { BLOCK, CMLNLE_NAMESPACE, CNXML_NAMESPACE, CXLXT_NAMESPACE, EDITING_NAMESPACE, INLINE, jsx$1 as JSX, LINE, Node$1 as Node, XMLNS_NAMESPACE, XML_NAMESPACE, buildElement, children, createElement, deserialize, jsx, jsxs, normalizeBlock, normalizeLine, normalizeVoid, render, serialize };
